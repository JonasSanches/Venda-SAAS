...
const CYCLES={MONTHLY:{code:"MONTHLY",label:"Mensal",months:1,discount:0,days:30,installments:1},SEMIANNUAL:{code:"SEMIANNUAL",label:"Semestral",months:6,discount:.2,days:180,installments:6},ANNUAL:{code:"ANNUAL",label:"Anual",months:12,discount:.3,days:365,installments:12}} as const;
type CycleCode=keyof typeof CYCLES;
...
export class BillingService{
  ...
  async checkout(tenantId:string,planCode:PlanCode,cycleCode:CycleCode,email?:string){
    ...
    const amount=Math.round(plan.amount*cycle.months*(1-cycle.discount)*100)/100,externalReference=randomUUID(),base=(process.env.PUBLIC_APP_URL??"https://www.vendamais-app.com").replace(/\/$/,"");
    const payment=await prisma.billingPayment.create({data:{tenantId,externalReference,amount,plan:`${plan.code}:${cycle.code}`}}),callback=`${base}/pagamento?cliente=${tenantId}&plano=${plan.code}&ciclo=${cycle.code}`;
    const response=await fetch("https://api.mercadopago.com/checkout/preferences",{method:"POST",headers:{Authorization:`Bearer ${this.token()}`,"Content-Type":"application/json","X-Idempotency-Key":externalReference},body:JSON.stringify({items:[{id:`vendamais-${plan.code.toLowerCase()}-${cycle.code.toLowerCase()}`,title:`Venda+ - ${plan.name} ${cycle.label}`,description:`${cycle.days} dias de acesso para ${tenant.name}`,quantity:1,currency_id:"BRL",unit_price:amount}],payer:email?{email}:undefined,external_reference:externalReference,notification_url:`${base}/api/billing/webhook`,back_urls:{success:`${callback}&resultado=sucesso`,pending:`${callback}&resultado=pendente`,failure:`${callback}&resultado=falha`},auto_return:"approved",payment_methods:{installments:cycle.installments},statement_descriptor:"VENDAMAIS"})});
  ...
  async webhook(paymentId:string){
    ...
    await prisma.$transaction(async tx=>{
      ...
      if(firstApproval){const[planCode,cycleCode="MONTHLY"]=String(payment.plan??"").split(":"),cycle=CYCLES[cycleCode as CycleCode]??CYCLES.MONTHLY,tenant=await tx.tenant.findUniqueOrThrow({where:{id:payment.tenantId},select:{subscriptionExpiresAt:true}}),now=new Date(),base=tenant.subscriptionExpiresAt&&tenant.subscriptionExpiresAt>now?tenant.subscriptionExpiresAt:now,expiresAt=new Date(base.getTime()+cycle.days*86_400_000);await tx.tenant.update({where:{id:payment.tenantId},data:{status:"ACTIVE",subscriptionPlan:planCode,subscriptionExpiresAt:expiresAt}})}
    });
