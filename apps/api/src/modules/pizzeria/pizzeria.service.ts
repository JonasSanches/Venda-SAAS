...
export class PizzeriaService {
  ...
  private assertOperator(){ const roles=tenantContext.getStore()?.roles??[]; if(!roles.some(role=>["ADMIN","MANAGER","SELLER","CASHIER","PLATFORM_ADMIN"].includes(role))) throw new ForbiddenException("Your role cannot operate pizza orders."); }
