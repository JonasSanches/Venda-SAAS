...
export class TrialService {
    ...
    try{
      const tenant=await prisma.$transaction(async tx=>{
        ...
        if(input.segment==="PIZZERIA"){
          ...
          await tx.pizzaSettings.create({data:{tenantId:created.id}});
        }
        if(input.segment==="TRANSPORTATION")await tx.tenantModule.create({data:{tenantId:created.id,module:"TRANSPORTATION",enabled:true}});
  ...
  async updateTenant(id:string,input:{name:string;phone:string;city:string;state:"SP"|"RJ";segment:string}){if(this.demoMode){const t=this.requireDemo(id),before={name:t.name,phone:t.phone,city:t.city,state:t.state,segment:t.segment};Object.assign(t,input);this.save();return{...this.publicDemo(t),before}}const before=await prisma.tenant.findUnique({where:{id},select:{name:true,phone:true,city:true,state:true,segment:true}});if(!before)throw new BadRequestException("Conta não encontrada");await prisma.$transaction(async tx=>{await tx.$executeRaw`SELECT set_config('app.tenant_id', ${id}, true)`;await tx.tenant.update({where:{id},data:{name:input.name.trim(),phone:input.phone.replace(/\D/g,""),city:input.city.trim(),state:input.state,segment:input.segment}});if(input.segment==="PIZZERIA"){await tx.tenantModule.upsert({where:{tenantId_module:{tenantId:id,module:"PIZZERIA"}},update:{enabled:true},create:{tenantId:id,module:"PIZZERIA",enabled:true}});await tx.pizzaSettings.upsert({where:{tenantId:id},update:{},create:{tenantId:id}})}if(input.segment==="TRANSPORTATION")await tx.tenantModule.upsert({where:{tenantId_module:{tenantId:id,module:"TRANSPORTATION"}},update:{enabled:true},create:{tenantId:id,module:"TRANSPORTATION",enabled:true}})});await this.audit(id,"TENANT_UPDATED","Tenant",id,before,input);return this.getDetail(id)}
  ...
  async setTenantPixKey(tenantId:string,pixKey:string,pixKeyType?:string){
    ...
    if(this.demoMode)return{message:"Chave Pix salva"};
    await prisma.tenant.update({where:{id:tenantId},data:{pixKey:pixKey.trim(),pixKeyType:pixKeyType?.trim()||null}});
    await this.audit(tenantId,"QR_PIX_KEY_UPDATED","Tenant",tenantId,null,{pixKeyUpdated:true,pixKeyType:pixKeyType?.trim()||null});
    return{message:"Chave Pix salva"};
  }
  async logSupportView(tenantId:string,actorId:string){if(this.demoMode)return;await prisma.auditLog.create({data:{tenantId,actorId,action:"PLATFORM_LIVE_VIEW_OPENED",resource:"Tenant",resourceId:tenantId,requestId:randomUUID(),after:{readOnly:true}}})}
