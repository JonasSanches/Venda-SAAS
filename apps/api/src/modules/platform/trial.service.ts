...
export class TrialService {
  ...
  async extend(id:string,days:number){
    ...
    if(this.demoMode){const t=this.requireDemo(id),current=t.expiresAt?new Date(t.expiresAt).getTime():Date.now();t.expiresAt=new Date(Math.max(Date.now(),current)+days*86_400_000).toISOString();if(t.status==="EXPIRED")t.status="TRIAL";this.save();return this.publicDemo(t)}
