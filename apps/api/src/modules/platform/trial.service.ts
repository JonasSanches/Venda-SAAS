...
type DemoTrial = { tenantId:string; name:string; document:string; state:"SP"|"RJ"; city:string; segment:string; phone:string; logoDataUrl?:string; branch:{name:string;state:string}; status:"TRIAL"|"ACTIVE"|"EXPIRED"|"SUSPENDED"; startsAt:string; expiresAt:string; limits:{users:number;branches:number;sales:number}; user:{id:string;tenantId:string;name:string;email:string;passwordHash:string;roles:string[];status?:"ACTIVE"|"BLOCKED"} };
...
export class TrialService {
  ...
  async extend(id:string,days:number){
    ...
    const base=Math.max(Date.now(),tenant.trialExpiresAt?.getTime()??Date.now()),trialExpiresAt=new Date(base+days*86_400_000);await prisma.tenant.update({where:{id},data:{trialExpiresAt,status:tenant.status==="EXPIRED"?"TRIAL":tenant.status}});await this.audit(id,"TRIAL_EXTENDED","Tenant",id,{trialExpiresAt:tenant.trialExpiresAt},{trialExpiresAt,days});return this.get(id);
  }
