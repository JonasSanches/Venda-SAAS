...
const allowed=new Set(["/","/teste","/pagamento","/biblioteca"]);
...
export class AnalyticsService{
  ...
  async report(daysInput:number,pageInput:number){
    ...
    const compact=await prisma.visitorEvent.findMany({where,select:{visitedAt:true,ipAddress:true},orderBy:{visitedAt:"asc"}}),total=compact.length,totalPages=Math.max(Math.ceil(total/pageSize),1),page=Math.min(Math.max(Number(pageInput)||1,1),totalPages);
    const visits=await prisma.visitorEvent.findMany({where,orderBy:{visitedAt:"desc"},skip:(page-1)*pageSize,take:pageSize}),uniqueVisitors=new Set(compact.map(item=>item.ipAddress).filter(Boolean)).size,dailyMap=new Map<string,number>(),formatter=new Intl.DateTimeFormat("en-CA",{timeZone:"America/Sao_Paulo",year:"numeric",month:"2-digit",day:"2-digit"}),todayKey=formatter.format(new Date());
