...
export class AnalyticsController{
  constructor(private readonly analytics:AnalyticsService,private readonly library:LibraryCurationService){}
  ...
  @Post("exclude-current-ip")excludeCurrentIp(@Req()request:PlatformRequest){if(request.identity?.tenantId!=="10000000-0000-4000-8000-000000000001"||!request.identity.roles.includes("PLATFORM_ADMIN"))throw new ForbiddenException("Acesso exclusivo da plataforma");return this.analytics.excludeCurrentIp(request)}
  ...
  @Get("library")libraryList(@Req()request:PlatformRequest){this.platformOnly(request);return this.library.list()}
  @Post("library/refresh")libraryRefresh(@Req()request:PlatformRequest){this.platformOnly(request);return this.library.refresh()}
  private platformOnly(request:PlatformRequest){if(request.identity?.tenantId!=="10000000-0000-4000-8000-000000000001"||!request.identity.roles.includes("PLATFORM_ADMIN"))throw new ForbiddenException("Acesso exclusivo da plataforma")}
}
