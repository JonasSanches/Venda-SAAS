...
export class CommercialService {
  ...
  async addParty(tenantId:string,input:{type:"CUSTOMER"|"SUPPLIER";name:string;document?:string;email?:string;phone?:string;address?:string;notes?:string}){const document=input.document?.replace(/\D/g,"")||undefined;if(this.demo){if(document&&this.data.parties.some((p:any)=>p.tenantId===tenantId&&p.document===document))throw new BadRequestException("Documento já cadastrado");const p={id:randomUUID(),tenantId,...input,document,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};this.data.parties.push(p);this.save();return p}try{return await withTenant(tenantId,tx=>tx.party.create({data:{tenantId,...input,name:input.name.trim(),document,email:input.email?.trim()||null,phone:input.phone?.trim()||null,address:input.address?.trim()||null,notes:input.notes?.trim()||null}}))}catch(e:any){if(e?.code==="P2002")throw new BadRequestException("Documento já cadastrado");throw e}}
