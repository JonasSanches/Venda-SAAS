...
export class AuthService {
  ...
  async createUser(input:{name:string;username:string;password:string;role:"ADMIN"|"MANAGER"|"SELLER"|"CASHIER"|"STOCK"}){
    const identity=tenantContext.getStore();if(!identity?.roles.some(role=>role==="ADMIN"||role==="MANAGER"))throw new ForbiddenException("Seu perfil não pode criar usuários");if(identity.roles.includes("MANAGER")&&!identity.roles.includes("ADMIN")&&!(["SELLER","CASHIER","STOCK"] as string[]).includes(input.role))throw new ForbiddenException("Gerentes podem criar somente vendedores, caixa ou estoque");const tenantId=currentTenantId(),username=input.username.trim().toLowerCase();if(!/^[a-z0-9._-]{3,30}$/.test(username))throw new BadRequestException("Nome de acesso deve ter de 3 a 30 caracteres: letras minúsculas, números, ponto, hífen ou sublinhado");const email=`${username}@acesso.vendamais-app.com`;
    ...
    const permissions={ADMIN:["*"],MANAGER:["sales:*","products:*","inventory:*","cash:*","customers:*","pizzeria:*","transportation:*"],SELLER:["sales:*","products:read","customers:*"],CASHIER:["sales:create","sales:read","cash:read"],STOCK:["products:read","inventory:*"]}[input.role];
  ...
  async setUserRole(userId:string,roleName:"ADMIN"|"MANAGER"|"SELLER"|"CASHIER"|"STOCK"){
    const identity=tenantContext.getStore();if(!identity?.roles.some(role=>role==="ADMIN"||role==="MANAGER"))throw new ForbiddenException("Seu perfil não pode alterar acessos");if(identity.roles.includes("MANAGER")&&!identity.roles.includes("ADMIN")&&!(["SELLER","CASHIER","STOCK"] as string[]).includes(roleName))throw new ForbiddenException("Gerentes podem definir somente vendedor, caixa ou estoque");const tenantId=currentTenantId();
    if(process.env.DEMO_MODE!=="false")return this.store.setUserRole(tenantId,userId,roleName);
    return withTenant(tenantId,async tx=>{const user=await tx.user.findFirst({where:{id:userId,tenantId},include:{roles:{include:{role:true}}}});if(!user)throw new BadRequestException("Usuário não encontrado");const current=user.roles.map(item=>item.role.name);if(current.includes("ADMIN")&&roleName!=="ADMIN"){const admins=await tx.user.count({where:{tenantId,status:"ACTIVE",roles:{some:{role:{name:"ADMIN"}}}}});if(admins<=1)throw new BadRequestException("Não é possível alterar o último administrador")};const permissions={ADMIN:["*"],MANAGER:["sales:*","products:*","inventory:*","cash:*","customers:*","pizzeria:*","transportation:*"],SELLER:["sales:*","products:read","customers:*"],CASHIER:["sales:create","sales:read","cash:read"],STOCK:["products:read","inventory:*"]}[roleName];const role=await tx.role.upsert({where:{tenantId_name:{tenantId,name:roleName}},update:{permissions},create:{tenantId,name:roleName,permissions}});await tx.userRole.deleteMany({where:{userId}});await tx.userRole.create({data:{userId,roleId:role.id}});return{message:"Acesso atualizado"}});
  }
